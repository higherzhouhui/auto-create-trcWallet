...
async function start() {
    try {
        ...
        class MemoryManager {
            ...
            shouldPerformGC() {
                const now = Date.now();
                return (now - this.lastGC) > (PERFORMANCE_CONFIG.MEMORY_CHECK_INTERVAL * 2);
            }
        }
        ...
        class Logger {
            ...
            async progress() {
                ...
                const rate = stats.totalGenerated / elapsed;
                const estimatedTime = (CONFIG.MAX_WALLETS - stats.totalGenerated) / rate;
        ...
        class WalletGenerator {
            ...
            async generateWallet() {
                try {
                    ...
                    if (isMatch) {
                        await this.saveMatchedWallet(walletData);
                        stats.totalMatched++;
            ...
            async processBatch() {
                ...
                const batch = this.batchQueue.splice(0, Math.min(CONFIG.BATCH_SIZE, this.batchQueue.length));
            ...
            async start() {
                ...
                try {
                    ...
                    const generateWithDelay = async () => {
                        if (!this.isRunning || stats.totalGenerated >= CONFIG.MAX_WALLETS) {
                            await this.stop();
                            return;
                        }
                        ...
                        try {
                            ...
                            if (this.batchQueue.length < PERFORMANCE_CONFIG.QUEUE_MAX_SIZE) {
                                this.batchQueue.push(Date.now());
                            } else {
                                await this.logger.log('队列已满，等待处理', 'WARN');
                            }
                            
                            // 如果队列达到批量大小，开始处理
                            if (this.batchQueue.length >= CONFIG.BATCH_SIZE && !this.processingBatch) {
                                await this.processBatch();
                            }
                            
                            // 继续生成
                            setTimeout(generateWithDelay, CONFIG.GENERATION_DELAY);
                            
                        } catch (error) {
                            this.errorManager.addError(error, 'generateWithDelay');
                            await this.logger.log(`生成过程中发生错误: ${error.message}`, 'ERROR');
                            // 即使出错也继续生成
                            setTimeout(generateWithDelay, CONFIG.GENERATION_DELAY);
                        }
                    };
                    ...
                    this.batchProcessor = setInterval(async () => {
                        if (this.isRunning && this.batchQueue.length > 0 && !this.processingBatch) {
                            await this.processBatch();
                        }
                    }, 100);
